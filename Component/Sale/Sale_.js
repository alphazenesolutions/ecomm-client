...
const Sale_ = () => {
  ...
  const getCategory_product = async () => {
    ...
    var myprodyct = await allproduct.data.filter((datanew) => {
      return datanew.store == storeid;
    });
    var Products_data = [];

    for (var i = 0; i < myprodyct.length; i++) {
      ...
      Products_data.push({
        ...
        category: myprodyct[i].category,
        name: myprodyct[i].name,
        price: myprodyct[i].price,
        slug: myprodyct[i].slug,
        original: myprodyct[i].original,
      });
    }
